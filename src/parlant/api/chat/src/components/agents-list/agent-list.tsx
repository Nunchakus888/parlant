...
import {agentAtom, agentsAtom, customerAtom, dialogAtom, newSessionAtom, sessionAtom} from '@/store';
...
const AgentList = (): ReactNode => {
	...
	const selectAgent = (agent: AgentInterface): void => {
		setAgent(agent);
		// Create a default customer for new sessions
		selectCustomer({id: 'guest', name: 'Guest'}, agent);
	};
			...
			<DialogHeader>
				<DialogTitle>
					<div className='mb-[12px] mt-[24px] w-full flex justify-between items-center ps-[30px] pe-[20px]'>
						<DialogDescription className='text-[20px] font-semibold'>Select an Agent</DialogDescription>
