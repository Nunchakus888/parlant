...
async def create_lab_results_journey(agent: p.Agent) -> p.Journey:
    # Create the journey
    journey = await agent.create_journey(
        title="Lab Results",
...
async def main() -> None:
    ...
    async def create_agent_for_customer(customer_id: p.CustomerId) -> p.Agent:
        # 注意：这里不能使用logger，因为logger还没有初始化
        print(f"为客户 {customer_id} 创建个性化智能体...")

        # 创建一个简单的agent对象
        # 注意：这是一个简化的实现，实际使用时可能需要更复杂的逻辑
        
        agent = p.Agent(
            id=p.AgentId(f"healthcare_agent_{customer_id}"),
        ...
        scheduling_journey = await create_scheduling_journey(agent)
        lab_results_journey = await create_lab_results_journey(agent)
